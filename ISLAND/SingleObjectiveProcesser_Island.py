...
if __name__ == '__main__':
	...
	for directory in directories:
		...
		for instance_index in range(num_islands):
			single_objective_result = RawSingleObjectiveResult(directory=directory, stat_file = ("stats_%i.txt" % instance_index))
			minimums_of_each_island.append([row[INDEX_OF_MINIMUM] for row in single_objective_result.statistics])
		...
		minimum_of_islands = []
		for i in range(len(minimums_of_each_island[0])):
			print(i,column(minimums_of_each_island,i))
			minimum_of_islands.append(np.min(column(minimums_of_each_island,i)))
		all_minimums_of_all_runs.append(minimum_of_islands)
