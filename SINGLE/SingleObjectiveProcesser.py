    ...
    def parse_stat_file(self):
        with open(self.stat_file, 'rb') as f:
            for generation in iter(f):
                self.fill_in_minimum_list(self.split_and_convert_values(generation))

    def split_and_convert_values(self, generation):
        return [float(value) for value in generation.split(", ")]

    def fill_in_minimum_list(self, generation):
        MIN_INDEX = 3
        current_minimum = generation[MIN_INDEX]
        if self.algorithm_name == 'Particle Swarm' or self.algorithm_name == 'Simulated Annealing':
            if len(self.minimums)==0:
                ...
                smoothed_minimums = self.smooth_minimums(current_minimum)
                self.minimums.append(smoothed_minimums)
        else:
            self.minimums.append(current_minimum)
...
if __name__ == '__main__':
    ...
    for instance_index, directory in enumerate(directories):
        ...
        all_minimums_of_all_runs.append(single_objective_result.minimums)
